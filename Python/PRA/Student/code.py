    ...
    def calculateStudentMarksUpdation(self,percent,stu_roll):
        for i in self.students_list:
            if(i.roll_no==stu_roll):
                ...
                return None
        return Student(i.roll_no,i.name,i.mark1,i.mark2,i.total_marks)
...
result = obj_school.calculateStudentMarksUpdation(reqd_percent,reqd_roll)

if(result != None):
    print("{0} {1} {2} {3}",result.name,result.mark1,result.mark2,result.total_marks)
else:
